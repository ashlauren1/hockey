...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function addSortToHeaders(tables) {
        const headers = tables.querySelectorAll("thead th");
        headers.forEach((header, index) => {
            header.style.cursor = "pointer";
            header.addEventListener("click", function () {
                sortTable(tables, index);
            });
        });
    }

    function sortTable(tables, columnIndex) {
        const rows = Array.from(tables.querySelectorAll("tbody tr"));
        const direction = tables.dataset.sortDirection === "desc" ? "asc" : "desc";
        tables.dataset.sortDirection = direction;
